    ...
    <div className=" flex gap-10 py-10 px-20">
      ...
      <div className="flex flex-col gap-2 w-full">
        ...
        <table style={{ borderCollapse: "separate", borderSpacing: "10px" }}>
          <tbody>
            <tr>
              <td>Combat: {heroes?.powerstats?.combat == 'null' ? 0 : heroes?.powerstats?.combat }</td>
              <td>Durability: {heroes?.powerstats?.durability == 'null' ? 0 : heroes?.powerstats?.durability}</td>
              <td>Intelligence: {heroes?.powerstats?.intelligence == 'null' ? 0 : heroes?.powerstats?.intelligence}</td>
            </tr>
            <tr>
              <td>Power: {heroes?.powerstats?.power == 'null' ? 0 : heroes?.powerstats?.power}</td>
              <td>Speed: {heroes?.powerstats?.speed == 'null' ? 0 : heroes?.powerstats?.speed}</td>
              <td>Strength: {heroes?.powerstats?.strength == 'null' ? 0 : heroes?.powerstats?.strength}</td>
            </tr>
          </tbody>
        </table>
        <Link
